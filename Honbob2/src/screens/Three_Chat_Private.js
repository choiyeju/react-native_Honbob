...
function updateChat(ref_chat2, boardId, userId) {
    var board2 = ref_chat2[userId].board;
    var ref_chat_board = [];
    console.log(board2);
    ...
    for(var i = 0; i < ref_chat_board.length; i++) {
        if (ref_chat_board[i] === String(boardId)) return;
    }
    board2 += boardId + ',';
...
function updateChatPri(ref_chatprivate2, boardId, userId) {
    var user2 = ref_chatprivate2[boardId].user
    var ref_chatprivate_user = [];
    if (user2 !== null) ref_chatprivate_user = user2.split(',');
    ref_chatprivate_user = ref_chatprivate_user.filter(function(item) {
        return item !== "";
    });
    for(var i = 0; i < ref_chatprivate_user.length; i++) {
        if (ref_chatprivate_user[i] === String(userId)) return;
    }
    user2 += userId + ',';
...
function removeChatPriChatF(navigation, boardId, userId) {
    ...
    ref_chat_board = ref_chat_board.split(',').filter(function(item) {
        return item !== "";
    });
    for (var i = 0; i < ref_chat_board.length; i++) {
...
function makeList2(ref_chatprivate2) {
    var c = [];
    for (var i = 0; i < ref_chatprivate2.length; i++) {
        c[i] = ref_chatprivate2[i] + " ";
    }
    return c;
}
...
export default class Three_Chat_Private extends Component
{
    ...
    updateChat(ref_chat, boardId, userId)
    updateChatPri(ref_chatprivate, boardId, userId)

    console.log("Chat_Private " + userId + " " + boardId);
    ...
    return (
        <Container>
            ...
            <View style={{height: '7%', backgroundColor: 'yellowgreen',}}>
                <View style={{flex:1,margin: 10, alignItems: 'flex-start', }}>
                    <Text style={{fontSize: 20,}}>{makeList2(ref_chatprivate[boardId].user.split(","))}</Text>
                </View>
            </View>
